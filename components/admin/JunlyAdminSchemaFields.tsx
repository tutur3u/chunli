...
import {
  getJunlyManifestCollectionSchema,
  type JunlySyncField,
} from "@/lib/junly-external-project-manifest";
...
function formatFieldValue(value: unknown, field: JunlySyncField) {
...
function parseFieldValue(value: string | boolean, field: JunlySyncField) {
  if (field.type === "boolean") {
    return value === true;
  }
...
export function JunlyAdminSchemaFields({
  collectionSlug,
  onChange,
  profileDataText,
}: {
  ...
  const schema = getJunlyManifestCollectionSchema(collectionSlug);
  const profileData = toRecord(profileDataText);
  const fields: JunlySyncField[] = [...(schema?.profileFields ?? [])];

  if (!schema || fields.length === 0 || !profileData) {
    return null;
  }

  const setField = (field: JunlySyncField, value: string | boolean) => {
