...
import { JunlyAdminMediaImage } from "@/components/admin/JunlyAdminMediaImage";
import { JunlyAdminSchemaFields } from "@/components/admin/JunlyAdminSchemaFields";
import { JunlyAdminSyncPanel } from "@/components/admin/JunlyAdminSyncPanel";
import type {
  JsonObject,
  JunlyAdminAsset,
  JunlyAdminCollection,
  JunlyAdminEntry,
  JunlyAdminStudioPayload,
  JunlyEntryStatus,
...
const ENTRY_STATUSES: JunlyEntryStatus[] = ["draft", "scheduled", "published", "archived"];

function mergeEntry(studio: JunlyAdminStudioPayload, entry: JunlyAdminEntry) {
  return {
    ...studio,
...
function mergeCollection(
  studio: JunlyAdminStudioPayload,
  collection: JunlyAdminCollection,
) {
  return {
    ...studio,
...
function getEntryDraft(entry: JunlyAdminEntry): EntryDraft {
  return {
...
function getCollectionDraft(collection: JunlyAdminCollection): CollectionDraft {
  return {
...
function getStatusClass(status: JunlyEntryStatus) {
  if (status === "published") {
...
function getCollectionTitle(collections: JunlyAdminCollection[], collectionId: string) {
  return collections.find((collection) => collection.id === collectionId)?.title ?? "Unsorted";
}

function getPrimaryAsset(entry: JunlyAdminEntry, assets: JunlyAdminAsset[]) {
  return assets
    ...
    <section className="grid min-h-[44rem] overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
      <div className="border-b border-slate-200 p-5 dark:border-slate-800 lg:border-r lg:border-b-0">
        <div className="relative flex aspect-[4/5] items-center justify-center overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-950">
          <JunlyAdminMediaImage alt={primaryAsset?.alt_text ?? entry.title} asset={primaryAsset} />
        </div>
          ...
          <Field label="Status">
            <select
              ...
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
                  status: event.target.value as JunlyEntryStatus,
                }))
              }
              value={draft.status}
            >
...
export function JunlyAdminClient({
  adminLinks,
  cmsBaseUrl,
  initialStudio,
  initialTarget,
  ...
  cmsBaseUrl: string;
  initialStudio: JunlyAdminStudioPayload;
  ...
  const [statusFilter, setStatusFilter] = useState<JunlyEntryStatus | "all">("all");
  ...
  const refreshStudio = async () => {
    ...
    try {
      setStudio(await fetchAdminJson<JunlyAdminStudioPayload>("/api/admin/studio"));
  ...
  const saveEntry = async (entryId: string, payload: Record<string, unknown>) => {
    ...
    setMutationError(null);
    try {
      const entry = await fetchAdminJson<JunlyAdminEntry>(`/api/admin/entries/${encodeURIComponent(entryId)}`, {
  ...
  const publishEntry = async (entryId: string, eventKind: "publish" | "unpublish") => {
    ...
    setMutationError(null);
    try {
      const entry = await fetchAdminJson<JunlyAdminEntry>(`/api/admin/entries/${encodeURIComponent(entryId)}/publish`, {
  ...
  const saveCollection = async (collectionId: string, payload: Record<string, unknown>) => {
    ...
    setMutationError(null);
    try {
      const collection = await fetchAdminJson<JunlyAdminCollection>(`/api/admin/collections/${encodeURIComponent(collectionId)}`, {
        body: JSON.stringify(payload),
        method: "PATCH",
      });
      ...
      <div className="mx-auto max-w-[96rem]">
        <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 dark:border-slate-800 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-black text-slate-900 dark:text-white md:text-5xl">Junly content</h1>
          </div>
          <div className="flex flex-wrap gap-2">
          ...
          <aside className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="border-b border-slate-200 p-4 dark:border-slate-800">
              <div className="grid gap-3">
                ...
                <select className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-sky-400 dark:border-slate-700 dark:bg-slate-950 dark:text-white" onChange={(event) => setStatusFilter(event.target.value as JunlyEntryStatus | "all")} value={statusFilter}>
                  ...
                  return (
                    ...
                    >
                      <div className="relative aspect-[4/5] overflow-hidden rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-800 dark:bg-slate-950">
                        <JunlyAdminMediaImage alt={entryAsset?.alt_text ?? entry.title} asset={entryAsset} />
                      </div>
