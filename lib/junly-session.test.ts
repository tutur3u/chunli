...
import type { JunlyAdminSession } from "./junly-session";
...
function createSession(): JunlyAdminSession {
  return {
...
function readSessionCookieValue(response: NextResponse) {
  ...
  expect(setCookie).toContain("junly_admin_session=");
  return setCookie?.split(";")[0]?.slice("junly_admin_session=".length) ?? "";
}

describe("junly session validation", () => {
  beforeEach(() => {
    process.env.TUTURUUU_API_BASE_URL = "https://platform.example.com/api/v1";
    process.env.TUTURUUU_JUNLY_WORKSPACE_ID = "ws-linked";
    process.env.JUNLY_SESSION_SECRET = "session-secret";
  ...
  afterEach(() => {
    ...
    delete process.env.TUTURUUU_JUNLY_WORKSPACE_ID;
    delete process.env.JUNLY_SESSION_SECRET;
    sessionCookieValue = null;
  });

  for (const status of [401, 403, 404] as const) {
    test(`rejects stored sessions when platform revalidation returns ${status}`, async () => {
      const { getJunlySessionFromCookies, setJunlySessionCookie } = await import(
        "./junly-session"
      );
      const response = NextResponse.json({});
      setJunlySessionCookie(response, createSession());
      ...
      await expect(getJunlySessionFromCookies()).resolves.toBeNull();
