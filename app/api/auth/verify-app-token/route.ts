...
import {
  ...
  type JunlyAdminSession,
} from "@/lib/junly-session";
...
function normalizeApiBaseUrl() {
  return getJunlyApiBaseUrl().replace(/\/+$/, "");
}
...
function toJunlySession(payload: AppTokenExchangeResponse): JunlyAdminSession {
  ...
  return {
    ...
    app: {
      name: payload.app?.name ?? getJunlyAppId(),
    },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const session = toJunlySession(await exchangeCrossAppToken(token));
    ...
    setJunlySessionCookie(response, session);
    return response;
  } catch (error) {
    console.error("[junly:auth] app token exchange failed", error);
